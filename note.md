...
# Factoring the Signup and Login error

We update our auth.service.ts:
...
# creating and storing the user data using a subject

We create the _User_ model which includes the token and its expiration date in milliseconds
...
Then create and store the user data using rxjs Subject. We create and store the user when signing up or loging in:
```
import { HttpClient, HttpErrorResponse } from '@angular/common/http';
import { Injectable } from '@angular/core';
...
We need to add the token bearer to the headers of output requests. That is we need to update our _fetchRecipes_ and _storeRecipes_ in the data-storage.service.ts file

To add the token we have to access the stored authenticated user from the auth.service.ts. 

```
Now we could set up a subcription to the user in the DataStorageService, but we don't care about every change to the user.
Instead when we store or fetch the recipes we just want to get the token of the currently authenticated user. We don't want
to have an ongoing subscription. 2 differeent approaches are possible:

1. store the token in the auth.service.ts when the user authenticates
2. switch from rxjs Subject to rxjs BehaviourSubject inside the auth.service.ts
```

## Approach 1: storing the token when the user authentitcate
...
## Approach 2: swithing from rxjs Subject to rxjs BehaviourSubject


The rxjs BehaviourSubject allows us to fetch the last authenticated user when we subscribe to it. As a result to fetch the last authenticated user we can simply subscribe and directly unsubscribe (i.e. there is no ongoing subscritpion when we use
this approach)
...
Then our updated data-storage.service.ts file becomes:
...
Here is what the exhaustMap would look like if the token had to be added to the query parameters instead (like what's the case in firebase)
```
    ...
    exhaustMap(user => {
        // we now add the token in the query parameters

        // we now return the observable so that the interested party can subscribe to it
        // To use the setRecipes we need to specify the type of the response data received from our http:
        return this.httpClient.get<Recipe[]>(
            this.urlRecipes + '?auth=' + user.token
        );
      }),
      ...
```
...
# Using the localStorage to store the last authenticated user and auto-login
...
We update the auth.servvice.ts file by 
1. adding the method _autoLogout_
2. updating the methods _handleAuthentication_, _autoLogin_ and _logout_ 
...
How can we prevent a user from visiting the url _localhost:4200/recipes_ if he is not authenticated

We do this by using a _route guard_

## Using the depricated CanActivate interface
...
### redirecting to _auth_ if the user is not authenticated: version 1

we update our auth.guard.ts as follows:
